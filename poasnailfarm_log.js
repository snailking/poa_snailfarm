...
var gameinfo_modal = document.getElementById("gameinfo_modal");
var gamemechanics_modal = document.getElementById("gamemechanics_modal");
var leaderboard_modal = document.getElementById("leaderboard_modal");
var event_modal = document.getElementById("event_modal");
var snail_modal = document.getElementById("snail_modal");
var egg_modal = document.getElementById("egg_modal");
var red_modal = document.getElementById("red_modal");
var spider_modal = document.getElementById("spider_modal");
var squirrel_modal = document.getElementById("squirrel_modal");
var tadpole_modal = document.getElementById("tadpole_modal");
var lettuce_modal = document.getElementById("lettuce_modal");
var carrot_modal = document.getElementById("carrot_modal");
var slug_modal = document.getElementById("slug_modal");
var acorn_modal = document.getElementById("acorn_modal");

// Array to close them all
var modalArray = [quest_modal, downtime_modal, wrongRound_modal, gameinfo_modal, gamemechanics_modal, leaderboard_modal, event_modal, snail_modal, egg_modal, red_modal, spider_modal, squirrel_modal, tadpole_modal, lettuce_modal, carrot_modal, slug_modal, acorn_modal];
...
/*
window.onclick = function(event) {
    if (event.target == downtime_modal || event.target == wrongRound_modal || event.target == quest_modal || event.target == gameinfo_modal) {
		CloseModal();
    }
}
...
function CloseModal() {
	...
	gameinfo_modal.style.display = "none";
	gamemechanics_modal.style.display = "none";
}
...
function gameinfoModal(){
	gameinfo_modal.style.display = "block";
}

function gamemechanicsModal(){
	gamemechanics_modal.style.display = "block";
}
...
function changeLogIteration(){
	var doc_iteration = document.getElementById('logiteration');
	if(logIteration == 40){
		logIteration = 10;
		doc_iteration.innerHTML = '<button type="button" class="btn btn-success" onclick="changeLogIteration()">Max Log: 10</button>';
	} else if(logIteration == 10){
		logIteration = 20;
		doc_iteration.innerHTML = '<button type="button" class="btn btn-success" onclick="changeLogIteration()">Max Log: 20</button>';
	} else if(logIteration == 20){
		logIteration = 40;
		doc_iteration.innerHTML = '<button type="button" class="btn btn-success" onclick="changeLogIteration()">Max Log: 40</button>';
	}
}
...
function updateProportionalCost() {
	...
	a_joinCost = (formatEthValue2(a_snailPot / 1000) + parseFloat(0.1)).toFixed(3);
	a_hatchCost = (formatEthValue2(a_snailPot / 5000) + parseFloat(0.02)).toFixed(3);
	a_carrotCost = (formatEthValue2(a_snailPot / 200) + parseFloat(0.5)).toFixed(3);
...
function webCheck(_func){
	if(a_gameActive == false){
		showModal(downtime_modal);
	} else if(a_playerRound != a_round){
		showModal(wrongRound_modal);
	} else {
		_func();
	}
}
	...
	myContract.allEvents({ fromBlock: startBlock, toBlock: 'latest' }).get(function(error, result){
		if(!error){
			...
			if(result.length > 0){ //check if we have events, if not stop the loop
				p_keepUpdating = true;
				for(i = 0; i < logIteration; i++){ //loop through only logIteration events at most
